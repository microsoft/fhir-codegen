...
public class Bundle_30_50 : ICrossVersionProcessor<Bundle>
{
    private Converter_30_50 _converter;
    internal Bundle_30_50(Converter_30_50 converter)
    {
    ...
    public void Process(ISourceNode node, Bundle current)
    {
        switch (node.Name)
        {
            ...
            case "link":
                current.Link.Add(Extract30BundleLinkComponent(node));
                break;

            case "entry":
                current.Entry.Add(Extract30BundleEntryComponent(node));
                break;
    ...
    private Bundle.LinkComponent Extract30BundleLinkComponent(ISourceNode parent)
    {
    ...
    private Bundle.EntryComponent Extract30BundleEntryComponent(ISourceNode parent)
    {
        ...
        foreach (ISourceNode node in parent.Children())
        {
            switch (node.Name)
            {
                ...
                case "resource":
                    ...
                    break;
                case "search":
                    current.Search = Extract30BundleSearchComponent(node);
                    break;
                case "request":
                    current.Request = Extract30BundleRequestComponent(node);
                    break;
                case "response":
                    current.Response = Extract30BundleResponseComponent(node);
                    break;
                // process inherited elements
                default:
                    _converter._backboneElement.Process(node, current);
    ...
    private Bundle.ResponseComponent Extract30BundleResponseComponent(ISourceNode parent)
    {
    ...
    private Bundle.RequestComponent Extract30BundleRequestComponent(ISourceNode parent)
    {
    ...
    private Bundle.SearchComponent Extract30BundleSearchComponent(ISourceNode parent)
    {
