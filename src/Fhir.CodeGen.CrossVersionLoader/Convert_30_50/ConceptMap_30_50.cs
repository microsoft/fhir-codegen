...
public class ConceptMap_30_50 : ICrossVersionProcessor<ConceptMap>, ICrossVersionExtractor<ConceptMap>
{
	private Converter_30_50 _converter;
	internal ConceptMap_30_50(Converter_30_50 converter)
	{
	...
	public void Process(ISourceNode node, ConceptMap current)
	{
		switch (node.Name)
		{
			...
			case "group":
				current.Group.Add(Extract30ConceptMapGroupComponent(node));
				break;
	...
	private ConceptMap.GroupComponent Extract30ConceptMapGroupComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "element":
					current.Element.Add(Extract30ConceptMapSourceElementComponent(node));
					break;

				case "unmapped":
					current.Unmapped = Extract30ConceptMapUnmappedComponent(node);
					break;
	...
	private ConceptMap.SourceElementComponent Extract30ConceptMapSourceElementComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "target":
					current.Target.Add(Extract30ConceptMapTargetElementComponent(node));
					break;
	...
	private ConceptMap.TargetElementComponent Extract30ConceptMapTargetElementComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "dependsOn":
					current.DependsOn.Add(Extract30ConceptMapOtherElementComponent(node));
					break;

				case "product":
					current.Product.Add(Extract30ConceptMapOtherElementComponent(node));
					break;
	...
	private ConceptMap.OtherElementComponent Extract30ConceptMapOtherElementComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "code":
					// element ConceptMap.group.element.target.dependsOn.code has been removed in the target spec
					break;
	...
	private ConceptMap.UnmappedComponent Extract30ConceptMapUnmappedComponent(ISourceNode parent)
	{
