...
namespace Fhir.CodeGen.CrossVersionLoader.Convert_30_50;

public class ValueSet_30_50 : ICrossVersionProcessor<ValueSet>, ICrossVersionExtractor<ValueSet>
{
	private Converter_30_50 _converter;
	internal ValueSet_30_50(Converter_30_50 converter)
	{
	...
	public void Process(ISourceNode node, ValueSet current)
	{
		switch (node.Name)
		{
			...
			case "compose":
				current.Compose = Extract30ValueSetComposeComponent(node);
				break;

			case "expansion":
				current.Expansion = Extract30ValueSetExpansionComponent(node);
				break;
	...
	private ValueSet.ComposeComponent Extract30ValueSetComposeComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "include":
					current.Include.Add(Extract30ValueSetConceptSetComponent(node));
					break;

				case "exclude":
					current.Exclude.Add(Extract30ValueSetConceptSetComponent(node));
					break;
	...
	private ValueSet.ConceptSetComponent Extract30ValueSetConceptSetComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "concept":
					current.Concept.Add(Extract30ValueSetConceptReferenceComponent(node));
					break;

				case "filter":
					current.Filter.Add(Extract30ValueSetFilterComponent(node));
					break;
	...
	private ValueSet.ConceptReferenceComponent Extract30ValueSetConceptReferenceComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "designation":
					current.Designation.Add(Extract30ValueSetDesignationComponent(node));
					break;
	...
	private ValueSet.DesignationComponent Extract30ValueSetDesignationComponent(ISourceNode parent)
	{
	...
	private ValueSet.FilterComponent Extract30ValueSetFilterComponent(ISourceNode parent)
	{
	...
	private ValueSet.ExpansionComponent Extract30ValueSetExpansionComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "parameter":
					current.Parameter.Add(Extract30ValueSetParameterComponent(node));
					break;

				case "contains":
					current.Contains.Add(Extract30ValueSetContainsComponent(node));
					break;
	...
	private ValueSet.ParameterComponent Extract30ValueSetParameterComponent(ISourceNode parent)
	{
	...
	private ValueSet.ContainsComponent Extract30ValueSetContainsComponent(ISourceNode parent)
	{
