...
public class StructureDefinition_43_50 : ICrossVersionProcessor<StructureDefinition>, ICrossVersionExtractor<StructureDefinition>
{
	private Converter_43_50 _converter;
	internal StructureDefinition_43_50(Converter_43_50 converter)
	{
    ...
    public void Process(ISourceNode node, StructureDefinition current)
	{
		switch (node.Name)
		{
			...
			case "mapping":
				current.Mapping.Add(Extract43StructureDefinitionMappingComponent(node));
				break;
			...
			case "context":
				current.Context.Add(Extract43StructureDefinitionContextComponent(node));
				break;
			...
			case "snapshot":
				current.Snapshot = Extract43StructureDefinitionSnapshotComponent(node);
				break;

			case "differential":
				current.Differential = Extract43StructureDefinitionDifferentialComponent(node);
				break;
	...
	private StructureDefinition.MappingComponent Extract43StructureDefinitionMappingComponent(ISourceNode parent)
	{
	...
	private StructureDefinition.ContextComponent Extract43StructureDefinitionContextComponent(ISourceNode parent)
	{
		StructureDefinition.ContextComponent current = new();
        ...
        foreach (ISourceNode node in parent.Children())
        {
            ...
            {
                case "type":
                    current.Type = Hl7.Fhir.Utility.EnumUtility.ParseLiteral<StructureDefinition.ExtensionContextType>(node.Text);
                    break;
	...
	private StructureDefinition.SnapshotComponent Extract43StructureDefinitionSnapshotComponent(ISourceNode parent)
	{
	...
	private StructureDefinition.DifferentialComponent Extract43StructureDefinitionDifferentialComponent(ISourceNode parent)
	{
