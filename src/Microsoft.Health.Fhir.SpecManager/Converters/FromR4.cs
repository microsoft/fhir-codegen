...
namespace Microsoft.Health.Fhir.SpecManager.Converters
{
    ...
    public sealed class FromR4 : IFhirConverter
    {
        ...
        private void AddConceptTree(
            ...
            string codeSystemId,
            List<fhir_4.CodeSystemConcept> concepts,
        ...
        private static bool TryGetTypeFromElements(
            string structureName,
            List<fhir_4.ElementDefinition> elements,
            out Dictionary<string, FhirElementType> elementTypes,
            out string regex)
        {
            elementTypes = null;
            ...
            try
            {
                ...
                foreach (fhir_4.ElementDefinition element in sd.Snapshot.Element)
                {
                    try
                    {
                        ...
                        {
                            foreach (fhir_4.Extension ext in element.Extension)
                            {
                ...
                {
                    foreach (fhir_4.ElementDefinitionConstraint con in sd.Differential.Element[0].Constraint)
                    {
                        ...
                        {
                            foreach (fhir_4.Extension ext in con.Extension)
                            {
                                switch (ext.Url)
                                {
            ...
            out FhirServerInfo serverInfo)
        {
            ...
            fhir_4.CapabilityStatement caps = metadata as fhir_4.CapabilityStatement;
            ...
            {
                fhir_4.CapabilityStatementRest rest = caps.Rest[0];

                if (rest.Interaction != null)
                {
                    foreach (fhir_4.CapabilityStatementRestInteraction interaction in rest.Interaction)
                    {
                ...
                {
                    foreach (fhir_4.CapabilityStatementRestResource resource in rest.Resource)
                    {
                ...
                {
                    foreach (fhir_4.CapabilityStatementRestResourceSearchParam sp in rest.SearchParam)
                    {
                ...
                {
                    foreach (fhir_4.CapabilityStatementRestResourceOperation operation in rest.Operation)
                    {
        ...
        private static FhirServerResourceInfo ParseServerRestResource(
            fhir_4.CapabilityStatementRestResource resource)
        {
            ...
            {
                foreach (fhir_4.CapabilityStatementRestResourceInteraction interaction in resource.Interaction)
                {
                    if (string.IsNullOrEmpty(interaction.Code))
                    {
                        continue;
            ...
            {
                foreach (fhir_4.CapabilityStatementRestResourceSearchParam sp in resource.SearchParam)
                {
            ...
            {
                foreach (fhir_4.CapabilityStatementRestResourceOperation operation in resource.Operation)
                {
