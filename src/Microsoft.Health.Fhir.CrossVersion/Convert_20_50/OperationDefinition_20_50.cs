...
public class OperationDefinition_20_50 : ICrossVersionProcessor<OperationDefinition>, ICrossVersionExtractor<OperationDefinition>
{
	private Converter_20_50 _converter;
	internal OperationDefinition_20_50(Converter_20_50 converter)
	{
	...
	public void Process(ISourceNode node, OperationDefinition current)
	{
		switch (node.Name)
		{
			...
			case "type":
                current.ResourceElement.Add(new Code<VersionIndependentResourceTypesAll>(Hl7.Fhir.Utility.EnumUtility.ParseLiteral<VersionIndependentResourceTypesAll>(node.Text)));
                current.TypeElement = new FhirBoolean(true);
				break;
			...
			case "parameter":
				current.Parameter.Add(Extract20OperationDefinitionParameterComponent(node));
				break;

			case "overload":
				current.Overload.Add(Extract20OperationDefinitionOverloadComponent(node));
				break;
	...
	private OperationDefinition.ParameterComponent Extract20OperationDefinitionParameterComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "binding":
					current.Binding = Extract20OperationDefinitionBindingComponent(node);
					break;

				case "part":
					current.Part.Add(Extract20OperationDefinitionParameterComponent(node));
					break;
	...
	private OperationDefinition.BindingComponent Extract20OperationDefinitionBindingComponent(ISourceNode parent)
	{
	...
	private OperationDefinition.OverloadComponent Extract20OperationDefinitionOverloadComponent(ISourceNode parent)
	{
