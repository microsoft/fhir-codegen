...
namespace Fhir.CodeGen.CrossVersionLoader.Convert_43_50;

public class ValueSet_43_50 : ICrossVersionProcessor<ValueSet>, ICrossVersionExtractor<ValueSet>
{
	private Converter_43_50 _converter;
	internal ValueSet_43_50(Converter_43_50 converter)
	{
	...
	public void Process(ISourceNode node, ValueSet current)
	{
		switch (node.Name)
		{
			...
			case "compose":
				current.Compose = Extract43ValueSetComposeComponent(node);
				break;

			case "expansion":
				current.Expansion = Extract43ValueSetExpansionComponent(node);
				break;
	...
	private ValueSet.ComposeComponent Extract43ValueSetComposeComponent(ISourceNode parent)
	{
        ...
        foreach (ISourceNode node in parent.Children())
        {
            switch (node.Name)
            {
                ...
                case "include":
                    current.Include.Add(Extract43ValueSetConceptSetComponent(node));
                    break;

                case "exclude":
                    current.Exclude.Add(Extract43ValueSetConceptSetComponent(node));
                    break;
	...
	private ValueSet.ConceptSetComponent Extract43ValueSetConceptSetComponent(ISourceNode parent)
	{
        ...
        foreach (ISourceNode node in parent.Children())
        {
            switch (node.Name)
            {
                ...
                case "concept":
                    current.Concept.Add(Extract43ValueSetConceptReferenceComponent(node));
                    break;

                case "filter":
                    current.Filter.Add(Extract43ValueSetFilterComponent(node));
                    break;
	...
	private ValueSet.ConceptReferenceComponent Extract43ValueSetConceptReferenceComponent(ISourceNode parent)
	{
        ...
        foreach (ISourceNode node in parent.Children())
        {
            switch (node.Name)
            {
                ...
                case "designation":
                    current.Designation.Add(Extract43ValueSetDesignationComponent(node));
                    break;
	...
	private ValueSet.DesignationComponent Extract43ValueSetDesignationComponent(ISourceNode parent)
	{
	...
	private ValueSet.FilterComponent Extract43ValueSetFilterComponent(ISourceNode parent)
	{
	...
	private ValueSet.ExpansionComponent Extract43ValueSetExpansionComponent(ISourceNode parent)
	{
        ...
        foreach (ISourceNode node in parent.Children())
        {
            switch (node.Name)
            {
                ...
                case "parameter":
                    current.Parameter.Add(Extract43ValueSetParameterComponent(node));
                    break;

                case "contains":
                    current.Contains.Add(Extract43ValueSetContainsComponent(node));
                    break;
	...
	private ValueSet.ParameterComponent Extract43ValueSetParameterComponent(ISourceNode parent)
	{
	...
	private ValueSet.ContainsComponent Extract43ValueSetContainsComponent(ISourceNode parent)
	{
