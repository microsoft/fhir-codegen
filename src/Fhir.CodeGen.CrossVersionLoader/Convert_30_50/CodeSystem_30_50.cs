...
public class CodeSystem_30_50 : ICrossVersionProcessor<CodeSystem>, ICrossVersionExtractor<CodeSystem>
{
	private Converter_30_50 _converter;
	internal CodeSystem_30_50(Converter_30_50 converter)
	{
	...
	public void Process(ISourceNode node, CodeSystem current)
	{
		switch (node.Name)
		{
			...
			case "filter":
				current.Filter.Add(Extract30CodeSystemFilterComponent(node));
				break;

			case "property":
				current.Property.Add(Extract30CodeSystemPropertyComponent(node));
				break;

			case "concept":
				current.Concept.Add(Extract30CodeSystemConceptDefinitionComponent(node));
				break;
	...
	private CodeSystem.FilterComponent Extract30CodeSystemFilterComponent(ISourceNode parent)
	{
	...
	private CodeSystem.PropertyComponent Extract30CodeSystemPropertyComponent(ISourceNode parent)
	{
	...
	private CodeSystem.ConceptDefinitionComponent Extract30CodeSystemConceptDefinitionComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "designation":
					current.Designation.Add(Extract30CodeSystemDesignationComponent(node));
					break;

				case "property":
					current.Property.Add(Extract30CodeSystemConceptPropertyComponent(node));
					break;
	...
	private CodeSystem.DesignationComponent Extract30CodeSystemDesignationComponent(ISourceNode parent)
	{
	...
	private CodeSystem.ConceptPropertyComponent Extract30CodeSystemConceptPropertyComponent(ISourceNode parent)
	{
