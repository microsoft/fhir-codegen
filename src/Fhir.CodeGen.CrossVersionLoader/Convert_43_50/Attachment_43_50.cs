...
public class Attachment_43_50 : ICrossVersionProcessor<Attachment>, ICrossVersionExtractor<Attachment>
{
	private Converter_43_50 _converter;
	internal Attachment_43_50(Converter_43_50 converter)
	{
	...
	public void Process(ISourceNode node, Attachment current)
	{
		switch (node.Name)
		{
			...
			case "size":
				current.SizeElement = new Integer64(_converter._primitive.GetIntOpt(node));
				break;

			case "_size":
				_converter._element.Process(node, current.SizeElement);
				break;
