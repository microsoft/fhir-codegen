...
public class Bundle_43_50 : ICrossVersionProcessor<Bundle>
{
    private Converter_43_50 _converter;
    internal Bundle_43_50(Converter_43_50 converter)
    {
    ...
    public void Process(ISourceNode node, Bundle current)
    {
        switch (node.Name)
        {
            ...
            case "link":
                current.Link.Add(Extract43BundleLinkComponent(node));
                break;

            case "entry":
                current.Entry.Add(Extract43BundleEntryComponent(node));
                break;
    ...
    private Bundle.LinkComponent Extract43BundleLinkComponent(ISourceNode parent)
    {
    ...
    private Bundle.EntryComponent Extract43BundleEntryComponent(ISourceNode parent)
    {
        ...
        foreach (ISourceNode node in parent.Children())
        {
            switch (node.Name)
            {
                ...
                case "resource":
                    ...
                    break;
                case "search":
                    current.Search = Extract43BundleSearchComponent(node);
                    break;
                case "request":
                    current.Request = Extract43BundleRequestComponent(node);
                    break;
                case "response":
                    current.Response = Extract43BundleResponseComponent(node);
                    break;
                // process inherited elements
                default:
                    _converter._backboneElement.Process(node, current);
    ...
    private Bundle.ResponseComponent Extract43BundleResponseComponent(ISourceNode parent)
    {
    ...
    private Bundle.RequestComponent Extract43BundleRequestComponent(ISourceNode parent)
    {
    ...
    private Bundle.SearchComponent Extract43BundleSearchComponent(ISourceNode parent)
    {
