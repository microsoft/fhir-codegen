...
public class ConceptMap_20_50 : ICrossVersionProcessor<ConceptMap>, ICrossVersionExtractor<ConceptMap>
{
	private Converter_20_50 _converter;
	internal ConceptMap_20_50(Converter_20_50 converter)
	{
	...
	public void Process(ISourceNode node, ConceptMap current)
	{
		switch (node.Name)
		{
			...
			case "group":
				current.Group.Add(Extract20ConceptMapGroupComponent(node));
				break;
	...
	private ConceptMap.GroupComponent Extract20ConceptMapGroupComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "element":
					current.Element.Add(Extract20ConceptMapSourceElementComponent(node));
					break;

				case "unmapped":
					current.Unmapped = Extract20ConceptMapUnmappedComponent(node);
					break;
	...
	private ConceptMap.SourceElementComponent Extract20ConceptMapSourceElementComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "target":
					current.Target.Add(Extract20ConceptMapTargetElementComponent(node));
					break;
	...
	private ConceptMap.TargetElementComponent Extract20ConceptMapTargetElementComponent(ISourceNode parent)
	{
		...
		foreach (ISourceNode node in parent.Children())
		{
			switch (node.Name)
			{
				...
				case "dependsOn":
					current.DependsOn.Add(Extract20ConceptMapOtherElementComponent(node));
					break;

				case "product":
					current.Product.Add(Extract20ConceptMapOtherElementComponent(node));
					break;
	...
	private ConceptMap.OtherElementComponent Extract20ConceptMapOtherElementComponent(ISourceNode parent)
	{
	...
	private ConceptMap.UnmappedComponent Extract20ConceptMapUnmappedComponent(ISourceNode parent)
	{
