...
namespace Microsoft.Health.Fhir.SpecManager.Language
{
    ...
    public sealed class CSharpBasic : ILanguage
    {
        ...
        private void WriteValueSet(
            FhirValueSet vs)
        {
            string vsName = FhirUtils.SanitizeForProperty(vs.Id ?? vs.Name, _reservedWords, NamingConvention.PascalCase);
            ...
            bool isResource)
        {
            ...
            {
                _writer.WriteLineIndented($"public class {complex.NameForExport(NamingConvention.PascalCase)} {{");
            }
            ...
            else if ((complex.Components != null) && complex.Components.ContainsKey(complex.Path))
            {
                _writer.WriteLineIndented(
                    $"public class" +
                        $" {complex.NameForExport(NamingConvention.PascalCase, true)}" +
                        $" :" +
                        $" {complex.TypeForExport(NamingConvention.PascalCase, _primitiveTypeMap, false)} {{");
            }
            else
            {
                _writer.WriteLineIndented(
                    $"public class" +
                        $" {complex.NameForExport(NamingConvention.PascalCase, true)}" +
                        $" :" +
                        $" {complex.TypeForExport(NamingConvention.PascalCase, _primitiveTypeMap)} {{");
            }
        ...
        private void WriteHeader()
        {
            ...
            _writer.WriteLineIndented($"  // Element Naming Style: {NamingConvention.PascalCase}");
            _writer.WriteLineIndented($"  // Enum Naming Style: {NamingConvention.PascalCase}");
